 ...
 * number of threads
 */
public abstract class FixnumLock implements Lock {
    ...
    public final boolean checkedTryLock() {
        if (isAuthorized(Thread.currentThread())) {
            return tryLock();
        } else {
            throw new OwnershipException("Thread is not authorized");
    ...
    public final void checkedLockInterruptibly() throws InterruptedException {
        if (isAuthorized(Thread.currentThread())) {
            lockInterruptibly();
        } else {
            throw new OwnershipException("Thread is not authorized");
    ...
    public final boolean checkedTryLock(long time, TimeUnit unit) throws InterruptedException {
        if (isAuthorized(Thread.currentThread())) {
